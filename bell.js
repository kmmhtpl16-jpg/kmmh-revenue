...
(function(){
  ...
  var REF_DAYS   = 90;   /* ย้อนหลังที่ไล่หา "คืนเงินสดไม่มีที่มา" */  var REF_FROM   = "2026-09-01";   /* เริ่มเตือน "คืนเงินสดยังไม่มีที่มา" ตั้งแต่วันนี้เป็นต้นไป — ก่อนหน้านี้ยังไม่ได้เก็บที่มา ไม่ต้องไล่ย้อน (ต้องตรงกับ REF_FROM ใน index.html) */  function refFromISO(t){ var d=shiftISO(t,-REF_DAYS); return (REF_FROM && d<REF_FROM) ? REF_FROM : d; }  var REFRESH_MS = 5*60*1000;
  ...
  async function collect(){
    ...
    }catch(e){ console.warn("bell gap", e); }    /* 5) คืนเงินสดที่ยังไม่มีที่มา — วันที่ยอดตรงแบบ "ไม่หักคืนเงิน" = เงินก้อนนั้นไม่ได้ถูกหักจากยอดขายวันนั้น */    try{      var refBad=[]; var _rpMap={}; gRefP.forEach(function(p){ var k=String(p.date).slice(0,10); (_rpMap[k]=_rpMap[k]||[]).push(p); });      gRef.forEach(function(a){        if(REF_FROM && String(a.date).slice(0,10)<REF_FROM) return;        if(a.status==="วันหยุด") return;        if(a.machine_net==null || a.form_main==null) return;        var rf=num(a.form_refund); if(rf<=0) return;        var mn=num(a.machine_net), fm=r2(num(a.form_main)+num(a.form_upk));        var _cc=0; (a.cns||[]).forEach(function(c){ _cc+=num(c&&c.cash); }); var exs=_cc; (a.ex||[]).forEach(function(e){ exs+=num(e&&e.amt); }); (_rpMap[String(a.date).slice(0,10)]||[]).forEach(function(p){ var v=num(p.amount); if(v>0 && v<=rf+1) exs+=v; }); if(exs>=rf-1) return; var _lf=r2(rf-exs); refBad.push({date:String(a.date).slice(0,10), rf:rf, left:_lf, cc:_cc, tie:(Math.abs(Math.abs(mn-r2(fm-rf))-_lf)<=1 && Math.abs(mn-r2(fm-rf))>1)});      });      if(refBad.length){        refBad.sort(function(a,b){ return a.date<b.date?1:-1; });        out.push({ key:"refund", icon:"\u21a9\ufe0f",          title:"คืนเงินสดยังไม่มีที่มา "+refBad.length+" วัน",          sub:"รวม "+TH(refBad.reduce(function(s,x){ return s+x.left; },0))+" บาท · คืนเงินสดที่ไม่มีใบลดหนี้/คำอธิบายรองรับ — เปิดวันนั้นแล้วกดปุ่ม \u21a9\ufe0f บันทึกที่มาเงินคืน ในกล่องเตือน",          items:refBad.slice(0,5).map(function(x){            return '<a href="javascript:void(0)" onclick="__bellGoDay(\''+x.date+'\')" style="color:#b45309;font-weight:700">'+beDate(x.date)+'</a> · คืนเงิน '+TH(x.rf)+(x.left<x.rf?(" · ขาด "+TH(x.left)):"")+(x.cc>0?(" · ใบลดหนี้คืนสด "+TH(x.cc)):"")+(x.tie?' <span style="color:#b91c1c">· = ยอดต่างของวันนั้น</span>':'');          }),          raw:true, more:Math.max(0,refBad.length-5), btn:null, act:null });      }    }catch(e){ console.warn("bell refund", e); }
